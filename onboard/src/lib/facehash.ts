export const DEFAULT_COLORS = ["#ff383c", "#ef4444", "#f97316", "#f59e0b", "#fb7185"];

const FACE_TYPES = ["round", "cross", "line", "curved"] as const;

type FaceType = (typeof FACE_TYPES)[number];

const FACE_SVG_DATA: Record<
  FaceType,
  {
    viewBox: string;
    paths: string[];
...
function clampSize(size: number | undefined): number {
  ...
  return Math.min(Math.max(Math.round(size), 16), 2000);
}
...
function computeFacehash(options: { name: string; colorsLength?: number }) {
  const safeColorsLength = options.colorsLength && options.colorsLength > 0
    ? options.colorsLength
    : DEFAULT_COLORS.length;

  const hash = stringHash(options.name);
  const faceIndex = hash % FACE_TYPES.length;
  const colorIndex = hash % safeColorsLength;
  const positionIndex = hash % SPHERE_POSITIONS.length;

  return {
    faceType: FACE_TYPES[faceIndex] ?? "round",
    colorIndex,
    rotation: SPHERE_POSITIONS[positionIndex] ?? { x: 0, y: 0 },
    initial: options.name.charAt(0).toUpperCase(),
  };
}
...
export function toFacehashSvg(options: FacehashOptions): string {
  const {
    name,
    size,
    variant = "gradient",
  ...
  const palette = colors && colors.length > 0 ? colors : DEFAULT_COLORS;
  const data = computeFacehash({ name, colorsLength: palette.length });
  const svgData = FACE_SVG_DATA[data.faceType];
  const background = backgroundColor ?? getColor(palette, data.colorIndex);

  const [, , vbWidth, vbHeight] = svgData.viewBox.split(" ").map(Number);
  const width = vbWidth ?? 1;
  const height = vbHeight ?? 1;
  const faceWidth = resolvedSize * 0.6;
  const faceHeight = faceWidth / (width / height);

  const centerX = resolvedSize / 2;
  const offsetMagnitude = resolvedSize * 0.05;
  const offsetX = data.rotation.y * offsetMagnitude;
  const offsetY = -data.rotation.x * offsetMagnitude;
  const x = centerX - faceWidth / 2 + offsetX;
  const y = resolvedSize / 2 - faceHeight / 2 + offsetY;
  const scale = faceWidth / width;

  const fontSize = resolvedSize * 0.26;
  const textY = y + faceHeight + resolvedSize * 0.08 + fontSize * 0.8;
  const facePaths = svgData.paths
    .map((path) => `<path d="${path}" fill="${escapeXml(foregroundColor)}" />`)
    .join("");

  const gradient = variant === "gradient"
    ? "<defs><radialGradient id=\"facehash-gradient\" cx=\"50%\" cy=\"50%\" r=\"80%\"><stop offset=\"0%\" stop-color=\"#ffffff\" stop-opacity=\"0.18\" /><stop offset=\"62%\" stop-color=\"#ffffff\" stop-opacity=\"0\" /></radialGradient></defs><rect width=\"100%\" height=\"100%\" fill=\"url(#facehash-gradient)\" />"
    : "";

  const text = showInitial && data.initial
    ? `<text x="${centerX + offsetX}" y="${textY}" text-anchor="middle" font-size="${fontSize}" font-family="${escapeXml(fontFamily)}" font-weight="${escapeXml(String(fontWeight))}" fill="${escapeXml(foregroundColor)}">${escapeXml(data.initial)}</text>`
    : "";

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${resolvedSize}" height="${resolvedSize}" viewBox="0 0 ${resolvedSize} ${resolvedSize}" role="img" aria-label="Facehash avatar"><rect width="100%" height="100%" fill="${escapeXml(background)}" />${gradient}<g transform="translate(${x} ${y}) scale(${scale})">${facePaths}</g>${text}</svg>`;
}

export function toFacehashSvgDataUri(options: FacehashOptions): string {
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(toFacehashSvg(options))}`;
}
